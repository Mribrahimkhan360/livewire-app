...
//    }
    ...
    {
        $OrderDetails = $this->orderRepository->findOrderDetail($id);
//        dd($OrderDetails);
//        dd($OrderDetails->toArray());



        return $this->orderRepository->findOrderDetail($id);
    }

    public function storeSale($orderId, $stockIds)
    {
        $userId = auth()->id();
        $firstStockId = $stockIds[0] ?? null;
        ...
        $this->orderRepository->storeSale([
            'user_id'   => $userId,
